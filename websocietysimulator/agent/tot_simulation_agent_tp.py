...
from .modules.memory_modules import MemoryTP
import logging
...
class TOTSimulationAgentTP(SimulationAgent):
    """TOT Simulation Agent with TP memory."""
    
    def __init__(self, llm: LLMBase):
        """Initialize TOTSimulationAgentTP"""
        super().__init__(llm=llm)
        logger = getattr(llm, 'logger', None)
        self.memory = MemoryTP(llm=self.llm, logger=logger)
