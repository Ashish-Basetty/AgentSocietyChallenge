...
from .modules.memory_modules import MemoryVoyager
import logging
...
class PlanningBaseline(PlanningBase):
    """Planning baseline identical to the original TOT agent."""
...
class TOTVoyagerSimulationAgent(SimulationAgent):
    """TOT-based simulation agent that uses Voyager memory summaries."""

    def __init__(self, llm: LLMBase):
        super().__init__(llm=llm)
        logger = getattr(llm, "logger", None)
        self.memory = MemoryVoyager(llm=self.llm, logger=logger)
        self.planning = PlanningBaseline(llm=self.llm, logger=logger)
        self.reasoning = ReasoningTOT(
            profile_type_prompt="", memory=self.memory, llm=self.llm, logger=logger
        )
        ...
        try:
            ...
            for sub_task in plan:
                if "user" in sub_task["description"]:
                    user_data = self.interaction_tool.get_user(
                        user_id=self.task["user_id"]
                    )
                    user = str(user_data) if user_data else "No user data available"
                elif "business" in sub_task["description"]:
                    item_data = self.interaction_tool.get_item(
                        item_id=self.task["item_id"]
                    )
                    business = (
                        str(item_data) if item_data else "No business data available"
                    )

            if not user:
                user = str(
                    self.interaction_tool.get_user(user_id=self.task["user_id"])
                    or "No user data available"
                )
            if not business:
                business = str(
                    self.interaction_tool.get_item(item_id=self.task["item_id"])
                    or "No business data available"
                )
            ...
            if not stars_line:
                ...
                try:
                    ...
                    stars = float(parts[1].strip()) if len(parts) > 1 else 3.0
            ...
            if not review_line:
                ...
                try:
                    ...
                    review_text = parts[1].strip() if len(parts) > 1 else "No review generated."
                except (ValueError, IndexError, AttributeError):
                    review_text = "No review generated."
